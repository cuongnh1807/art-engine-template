...
export class ArtNFTEngine {
  private calculateDna(attributes: TAttribute[]) {
    const dnaSource = attributes.sort((a, b) => {
    ...
    for (const layer of layers) {
      if (bool(layer.probability)(mt)) {
        ...
        if (selected) {
          ...
          images.push({
            zIndex: layer.zIndex,
            filename: path.join(layersPath, selected.file),
          });
        }
      }
    }
...
(async () => {
  ...
  while (count < numberGen) {
    ...
    // save the dna data
    fs.writeFileSync(
      path.join(outputPath, `${dna}.json`),
      JSON.stringify(selection.selectedTraits)
    );
  }
})();
